...
class Automation
{
    ...
    unordered_map<State, vector<State>> table = {
        {State::start,
         {State::start, State::singed, State::in_number, State::end}},
        {State::singed, {State::end, State::end, State::in_number, State::end}},
        {State::in_number,
         {State::end, State::end, State::in_number, State::end}},
        {State::end, {State::end, State::end, State::end, State::end}},
    };

    // 获取字符串的类型，对应在table中的column
    int get_col(char c)
    {
        if (isspace(c)) {
            return 0;
