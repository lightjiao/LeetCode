    ...
    void gameOfLife(vector<vector<int>>& board)
    {
        ...
        for (int i = 0; i < board.size(); i++) {
            for (int j = 0; j < board[i].size(); j++) {
                int count = 0;
                for (auto [move_i, move_j] : move) {
                    if (i + move_i >= board.size() || i + move_i < 0 ||
                        j + move_j >= board[i].size() || j + move_j < 0) {
                        continue;
                    }
                    if (temp[i + move_i][j + move_j] == 1) {
                        count++;
                    }
                }
